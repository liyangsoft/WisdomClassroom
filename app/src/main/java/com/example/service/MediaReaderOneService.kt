...
class MediaReaderOneService : Service() {
...
//        voiceSender.exit()
    ...
    private inner class SendThread : SocketServerThread(Constants.groupPort1) {
        override fun onError(t: Throwable) {
//            myApplication.serverStatus = ServerStatus.UNSTART
            handler.post {
                Toast.makeText(this@MediaReaderOneService, "${"服务开启失败"}:${t.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
