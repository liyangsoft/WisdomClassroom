...
class MediaReaderThreeService : Service() {

    companion object {
        //        private val TAG = MediaReaderService::class.java.simpleName
        private val TAG = "MediaReaderService"
    ...
    private inner class SendThread : SocketServerThread(Constants.groupPort3) {
        override fun onError(t: Throwable) {
//            myApplication.serverStatus = ServerStatus.UNSTART
            handler.post {
                Toast.makeText(this@MediaReaderThreeService, "${"服务开启失败"}:${t.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
