...
class MediaReaderTwoService : Service() {
    ...
    private inner class SendThread : SocketServerThread(Constants.groupPort2) {
        override fun onError(t: Throwable) {
//            myApplication.serverStatus = ServerStatus.UNSTART
            handler.post {
                Toast.makeText(this@MediaReaderTwoService, "${"服务开启失败"}:${t.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
