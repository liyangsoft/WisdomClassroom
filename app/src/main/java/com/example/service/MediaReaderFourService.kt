...
class MediaReaderFourService : Service() {
...
//        voiceSender.exit()
    ...
    private inner class SendThread : SocketServerThread(Constants.groupPort4) {
        override fun onError(t: Throwable) {
//            myApplication.serverStatus = ServerStatus.UNSTART
            handler.post {
                Toast.makeText(this@MediaReaderFourService, "${"服务开启失败"}:${t.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
